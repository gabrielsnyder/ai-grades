...
// Format office label: Senate stays "Senate"; House uses "ST-N" (e.g. "NM-3", "AK-AL")
function officeLabel(fecOffice, state, district) {
  if (fecOffice === 'S') return 'Senate'
  ...
  try {
    for (const office of offices) {
      ...
      for (const fc of fecCandidates) {
        ...
        const existing = await prisma.candidate.findFirst({ where: { externalId } })
        if (existing) {
