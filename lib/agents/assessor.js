...
export async function runAssessorFromFinding({ candidateId, finding, provider = 'minimax' }) {
  ...
  // BILL_VOTE indicators are shared across candidates (one indicator per bill).
  // Statement/custom indicators are candidate-scoped: prefix the key with candidateId
  // so "Espaillat statement on X" created while researching Espaillat never appears
  // under a different candidate's record.
  const isBillVote = finding.type === 'BILL_VOTE'
  const indicatorKey = isBillVote ? finding.indicatorName : `${candidateId}::${finding.indicatorName}`

  const indicator = await prisma.indicator.upsert({
    where: { questionId_type_name: { questionId: finding.questionId, type: finding.type, name: indicatorKey } },
    create: {
