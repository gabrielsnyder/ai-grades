...
export async function runResearcher({ candidateId, questionIds, provider = 'minimax', dryRun = false }) {
  ...
  try {
    ...
    // Serper snippets are ~150 chars — too short to confirm involvement. Strategy:
    // 1. Always fetch the top 2 results from the broadest "ai" search (no name filter
    //    needed — if the query returns H.R.7226, Bean IS in it even if the snippet says
    //    "Mr. Moore of Utah (for himself and Mr....")
    // 2. Also fetch any additional results that explicitly name the candidate + AI
    const allOrganic = searchResults.flatMap(s => s.organic)
    const aiSearch = searchResults.find(s => s.key === 'ai')?.organic ?? []
    const nameLower = candidate.name.toLowerCase()

    const skipDomains = /linkedin\.com|twitter\.com|x\.com|facebook\.com|wikipedia\.org|youtube\.com|youtu\.be/

    // Top 2 from broad AI search — always fetch regardless of name in snippet
    const topAiUrls = aiSearch
      .filter(r => r.link && !skipDomains.test(r.link))
      .slice(0, 2)
      .map(r => r.link)

    // Additional pages where candidate name + AI appears in the snippet
    const namedUrls = allOrganic
      .filter(r => {
        if (!r.link || extractYoutubeId(r.link) || skipDomains.test(r.link)) return false
        if (topAiUrls.includes(r.link)) return false
        const txt = `${r.title ?? ''} ${r.snippet ?? ''}`.toLowerCase()
        return txt.includes(nameLower.split(' ').pop()) &&
          /\bai\b|artificial intelligence|algorithm/.test(txt)
      })
      .map(r => r.link)

    const uniquePageUrls = [...new Set([...topAiUrls, ...namedUrls])].slice(0, 4)
    const fetchedPages = (
