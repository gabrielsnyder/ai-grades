...
// Search the full page text for every mention of the candidate's name and
// return up to maxExcerpts windows of context around each hit.
// If the name isn't found, returns null — so we skip pages where Google
// surfaced a topic match but the candidate isn't actually mentioned.
function extractNameExcerpts(pageText, candidateName, windowChars = 600, maxExcerpts = 3) {
  const lastName = candidateName.split(' ').pop()
  ...
  for (const pos of positions) {
    ...
    const end = Math.min(pageText.length, pos + half)
    if (start >= lastEnd) {
      excerpts.push(pageText.slice(start, end).trim())
      lastEnd = end
...
export async function runResearcher({ candidateId, questionIds, provider = 'minimax', dryRun = false }) {
  ...
  try {
    ...
    // Fetch the top non-social organic results, then search each page's text
    // for the candidate's name. If found, extract context windows around each
    // mention. If not found, skip — Google ranked it for the query but the
    // candidate isn't discussed there.
    const allOrganic = searchResults.flatMap(s => s.organic)
    const skipDomains = /linkedin\.com|twitter\.com|x\.com|facebook\.com|wikipedia\.org|youtube\.com|youtu\.be/

    const candidatePageUrls = [...new Set(
      allOrganic
        .filter(r => r.link && !skipDomains.test(r.link) && !extractYoutubeId(r.link))
        .map(r => r.link)
    )].slice(0, 6)

    const fetchedPages = (
      await Promise.all(candidatePageUrls.map(async url => {
        const pageText = await fetchPageText(url)
        if (!pageText) return null
        const excerpts = extractNameExcerpts(pageText, candidate.name)
    ...
    const searchCtx = [
      ...
      ...(fetchedPages.length > 0
        ? fetchedPages.map(p => `### Excerpts from ${p.url} (passages mentioning candidate)\n${p.excerpts}`)
        : []
      ),
