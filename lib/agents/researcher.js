...
export async function runResearcher({ candidateId, questionIds, provider = 'minimax', dryRun = false }) {
  ...
  if (dryRun) {
    return {
      ...
      chamber,
      queries: [
        ...questions.map(q => `${candidate.name} ${candidate.state} ${role} "${q.shortLabel || q.text}" AI policy`),
  ...
  try {
    ...
    for (const q of questions) {
      const query = `${candidate.name} ${candidate.state} ${role} "${q.shortLabel || q.text}" AI`
    ...
    // Broad bill-vote sweep — chamber-specific bill prefix so we get the right floor votes
    const voteQuery = `${candidate.name} ${candidate.state} ${role} AI ${billPrefix} bill vote congress`
