          ...
          properties: {
            ...
            meta: {
              ...
              properties: {
                ...
                yesMeans:     { type: 'integer', minimum: 1, maximum: 5, description: 'What a YES vote implies on the 1–5 skeptic↔booster scale. 1 = YES means the candidate opposes AI deployment/favors heavy regulation. 5 = YES means the candidate supports rapid AI advancement/minimal restriction. Use the bill\'s actual policy direction.' }
              }
            },
            sourceUrl:   { type: 'string' },
            sourceTitle: { type: 'string' },
            excerpt:     { type: 'string',  description: 'Relevant quote or description from the source' },
            confidence:  { type: 'number',  description: '0–1 confidence this evidence is accurate and relevant. Use 0.4–0.5 for circumstantial evidence (e.g. bill appeared in candidate-name search but snippet cut off). Use 0.7+ for explicit confirmed evidence.' }
          },
...
async function serperSearch(query, candidateId, { news = false } = {}) {
  const cacheKey = news ? `[news] ${query}` : query
  ...
  const url = news ? SERPER_NEWS_URL : SERPER_SEARCH_URL
  ...
  await prisma.searchCache.create({
    data: {
      query: cacheKey, normalizedQuery, results,
      resultCount: (results.organic ?? results.news)?.length ?? 0,
...
// Format Serper results into a compact string for the LLM.
// Includes organic hits, news articles, answer box, and people-also-ask.
function formatSerperResults(results) {
  ...
  const organicHits = results.organic ?? []
  if (organicHits.length) {
    parts.push(organicHits.map(r =>
      `- [${r.title}](${r.link})\n  ${r.snippet ?? '(no snippet)'}`
    ).join('\n'))
  }

  const newsHits = results.news ?? []
  if (newsHits.length) {
    parts.push(newsHits.map(r =>
      `- [${r.title}](${r.link}) — ${r.date ?? ''}\n  ${r.snippet ?? '(no snippet)'}`
    ).join('\n'))
  }

  const paa = results.peopleAlsoAsk ?? []
  if (paa.length) {
    parts.push('**People also ask:**\n' + paa.slice(0, 3).map(q =>
      `Q: ${q.question}\nA: ${q.snippet ?? q.answer ?? '(no answer)'}`
    ).join('\n'))
  }
...
export async function runResearcher({ candidateId, questionIds, provider = 'minimax', dryRun = false }) {
  ...
  if (dryRun) {
    return {
      ...
      queries: [
        ...
        `${candidate.name} site:congress.gov`,
        `[news] ${candidate.name} artificial intelligence`,
      ],
    }
  }
  ...
  try {
    ...
    const broadSearches = [
      ...
      {
        // Congress.gov search surfaces bill sponsorships/co-sponsorships directly.
        // A co-sponsor's name appears on the bill page even if not in Serper snippets.
        key: 'congress',
        query: `${candidate.name} site:congress.gov`,
      },
    ]
    ...
    for (const s of broadSearches) {
      ...
      searchResults.push({ key: s.key, query: s.query, results })
    }

    // News search — returns longer, more informative snippets from journalism
    const newsQuery = `${candidate.name} artificial intelligence`
    const { results: newsResults, fromCache: newsFromCache } = await serperSearch(newsQuery, candidateId, { news: true })
    newsFromCache ? cacheHitCount++ : searchCount++
    ...
    const searchCtx = [
      ...searchResults.map(({ query, results }) =>
        `### Search: "${query}"\n${formatSerperResults(results)}`
      ),
      `### News search: "${newsQuery}"\n${formatSerperResults(newsResults)}`,
    ].join('\n\n')
    ...
    const response = await client.messages.create({
      ...
      system: [
        {
          ...
          text: [
            ...
            '',
            'CANDIDATE SCOPE (critical):',
            `- You are researching ${candidate.name} ONLY. Every finding must describe an action or statement by ${candidate.name} personally.`,
            '- If a search result mentions another legislator\'s statement, vote, or bill sponsorship — even prominently — DO NOT record it. Skip it entirely.',
            '- The search results may contain articles about other members of Congress. Ignore those; only extract evidence where the subject is the candidate being researched.',
            '',
            'General rules:',
            '- Record findings with confidence as low as 0.4 — it is better to record uncertain evidence than to record nothing.',
            '- Do not fabricate specific facts (votes, quotes). But you MAY infer involvement from circumstantial evidence:',
            `  * If a bill about AI appears in search results returned for ${candidate.name}'s name, the candidate is almost certainly a sponsor or co-sponsor. Record it as BILL_VOTE or CUSTOM with confidence 0.4–0.5 and note in the excerpt that the exact role (sponsor vs co-sponsor) needs verification.`,
            '  * If a news article about AI policy appears in results for this candidate, record the article\'s general thrust as a PUBLIC_STATEMENT with confidence 0.4–0.5.',
            ...
            '- DEDUPLICATION: Each bill or statement must appear as AT MOST ONE finding. Assign it to the single most relevant questionId. Do not record the same bill or statement under multiple questions.',
            '- Minimum confidence: 0.4. Only skip findings you are near-certain are irrelevant or wrong.',
          ].join('\n'),
      ...
      messages: [{
        ...
        content: [
          ...
          searchCtx,
          '',
          `Extract all relevant evidence about ${candidate.name} specifically. Each piece of evidence must appear in exactly one finding assigned to its most relevant questionId. Do not record other legislators' actions.`,
        ].join('\n'),
      }],
    })
    ...
    for (const f of rawFindings) {
      if ((f.confidence ?? 0) < 0.4) continue
